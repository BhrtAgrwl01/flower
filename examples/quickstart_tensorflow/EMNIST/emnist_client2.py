...
if __name__ == "__main__":
    ...
    for alpha_val in sys.argv[1:]:
        ...
        data2 = np.load('second_splitted_dataset.npz', allow_pickle=True)
    ...
    final_accuracies_client2 = [accuracy_vals[11*(n+1)-1] for n in range(len(sys.argv)-1)]
    print(final_accuracies_client2)

    plt.plot(sys.argv[1:], final_accuracies_client2, "ro-")
    plt.title("Client 2 accuracies")
    plt.xlabel("Alpha")
    plt.ylabel("Accuracy")
    plt.yticks(np.arange(max(min(final_accuracies_client2)-0.1, 0), min(max(final_accuracies_client2)+0.1, 1), 0.05))
    plt.show(block=True)
